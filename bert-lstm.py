...
# 当使用FA\SF\CF的分类总数作为下方类别数时 造成device-side assert报错 原因见README.md
output_size = 1577       # 此处为分类 类别数
hidden_dim = 768
n_layers = 2
bidirectional = True
...
result_comments = pretreatment(list(data['Sequence'].values))
...
y = torch.from_numpy(data['CFF'].values).float()
